...
첫 번째 음성은 원본 배우의 연기이고, 두 번째 음성은 플레이어가 따라한 것입니다.

명대사: "${sentence}"

원본 배우의 연기를 기준으로 플레이어가 얼마나 잘 따라했는지 평가해주세요.
player1은 원본(배우), player2는 플레이어입니다.

다음 4가지 기준으로 각각 0~100점으로 평가해주세요:
1. 어조/분위기 (Tone): 원본의 어조와 분위기를 얼마나 잘 재현했는가 (가중치 40%)
2. 감정이입 (Emotion): 원본의 감정을 얼마나 진실되게 따라했는가 (가중치 30%)
3. 리듬/표현력 (Rhythm): 원본의 억양, 강약, 리듬감을 얼마나 잘 살렸는가 (가중치 20%)
4. 발음 (Pronunciation): 발음이 얼마나 정확한가 (가중치 10%)

총점 계산: (tone * 0.4) + (emotion * 0.3) + (rhythm * 0.2) + (pronunciation * 0.1)

player1(원본)은 기준이므로 높은 점수(90~100)를 부여하세요.
player2(플레이어)는 모방도에 따라 0~100점으로 채점하세요.
winner는 총점이 더 높은 쪽으로 설정하세요.
...
반드시 아래 JSON 형식으로만 응답하세요:
{
  ...
  "player1_feedback": "원본 배우 연기에 대한 코멘트",
  "player2_feedback": "플레이어의 모방에 대한 피드백"
}`;
